...
def ApplyAlpha(Foreground, Offset, Operator, Background, currentY, FreeLine, TestEntity):
        #0 = transparent, 255 = opaque
        #Create PictureOut as an output buffer
        PictureOut = np.zeros((len(Foreground[currentY]), 4), dtype=np.uint8)

        #Get the offset of the foreground and background



        #For each pixel in the picture, apply the alpha value
        if (Operator == ("Over" or "over")):
            for CurrentX in range (Offset[1], Offset[1]+len(Foreground[currentY])):

                #If the current pixel is free
                if FreeLine[CurrentX] == True:


                    if Foreground[currentY][CurrentX-Offset[1]][3] == 0:
                        PictureOut[CurrentX] = Foreground[currentY][CurrentX-Offset[1]]
                        ...
                        Alpha = np.asarray(Foreground[currentY][CurrentX-Offset[1]][3])
                        AlphaOut = Alpha
                        Alpha = Alpha/255
                        ...
                        Red = AlphaFormula(Foreground[currentY][CurrentX-Offset[1]][0], Background[currentY][CurrentX][0], Alpha)
                        TestEntity.ApplyAlphaR += 1
                        Green = AlphaFormula(Foreground[currentY][CurrentX-Offset[1]][1], Background[currentY][CurrentX][1], Alpha)
                        TestEntity.ApplyAlphaG += 1
                        Blue = AlphaFormula(Foreground[currentY][CurrentX-Offset[1]][2], Background[currentY][CurrentX][2], Alpha)
